...
/* ============================== Types ============================== */
/* track.json only needs videoId + duration (fallback, until the player
   reports the real one). Title, artist and thumbnail are pulled live from
   YouTube — title/artist via the oEmbed endpoint, thumbnail via YouTube's
   own i.ytimg.com URL (never downloaded or re-hosted). */
...
function thumbnailFor(videoId: string): string {
  // YouTube's own hosted thumbnail — hotlinked, never downloaded/re-hosted.
  return `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
}

/* Public, unauthenticated endpoint — no API key needed. Returns { title, author_name, ... } */
async function fetchOEmbed(videoId: string): Promise<{ title: string; author_name: string } | null> {
  try {
...
function QueueDrawer({
  ...
  heading: string;
  metaFor: (videoId: string) => VideoMeta;
}) {
  if (!open) return null;
  return (
...
export default function MusicPlayer({ data }: { data: TracksData }) {
  ...
  const [metaCache, setMetaCache] = useState<Record<string, VideoMeta>>({});
  ...
  const metaFor = useCallback(
    (videoId: string): VideoMeta =>
      metaCache[videoId] ?? { title: "", author: "", thumbnail: thumbnailFor(videoId) },
    [metaCache]
  );

  /* ---- fetch title/artist for every track in the visible queue via oEmbed ---- */
  useEffect(() => {
    let cancelled = false;
    queue.forEach((t) => {
      if (metaCache[t.videoId]) return;
      setMetaCache((prev) => ({
        ...prev,
        [t.videoId]: { title: "", author: "", thumbnail: thumbnailFor(t.videoId) },
      }));
      fetchOEmbed(t.videoId).then((res) => {
  ...
  /* ---- fallback: if oEmbed hasn't resolved yet, take title/author straight
     from the IFrame player itself once it has loaded the video ---- */
  const captureVideoData = useCallback((player: any, videoId?: string) => {
    if (!videoId || typeof player.getVideoData !== "function") return;
    const vd = player.getVideoData();
    if (!vd?.title) return;
    setMetaCache((prev) => {
      if (prev[videoId]?.title) return prev; // oEmbed already filled it in
      return {
        ...prev,
        [videoId]: { title: vd.title, author: vd.author ?? "", thumbnail: thumbnailFor(videoId) },
      };
    });
  }, []);
      ...
      ytPlayerRef.current = new YT.Player(mountId, {
        ...
        events: {
          onReady: (e: any) => {
            ...
            captureVideoData(e.target, currentTrack?.videoId);
  ...
  const nowPlayingMeta = currentTrack ? metaFor(currentTrack.videoId) : { title: "", author: "", thumbnail: "" };
        ...
        <div className="glass flex w-full items-center gap-4 rounded-full p-3 pr-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline justify-between gap-2">
              <p className="truncate text-[15px] font-semibold text-white">{nowPlayingMeta.title || "Loading…"}</p>
              <QueueButton count={queue.length} onClick={() => setQueueOpen(true)} />
            </div>
            <p className="truncate text-[12.5px] text-white/70">{nowPlayingMeta.author}</p>
